...
class Videollama2MetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal_with_visiontoken(
        self, input_ids, attention_mask, past_key_values, labels, mm_features
    ):
        # vision_tower = self.get_vision_tower()
        # # NOTE: text-only situation
        if mm_features is None or input_ids.shape[1] == 1:
            # if past_key_values is not None and vision_tower is not None and Xs is not None and input_ids.shape[1] == 1:
            #    attention_mask = torch.ones((attention_mask.shape[0], past_key_values[-1][-1].shape[-2] + 1), dtype=attention_mask.dtype, device=attention_mask.device)
            return input_ids, attention_mask, past_key_values, None, labels
        #
